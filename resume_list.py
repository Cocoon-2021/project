...
async def list_all_resume(request): # --- function get list of resumes
    ...
    fetch_resumes_query = """select 
                                ...
                                "skills",(select json_arrayagg(json_object("name", skills.name, "level", skills.level, "keywords", skills.keywords)) from skills where basics_information.id = skills.resumeId),
                                "languages",(select json_arrayagg(json_object("language", languages.language, "fluency", languages.fluency)) from languages where basics_information.id = languages.resumeId),
                                "interests",(select json_arrayagg(json_object("name", interests.name, "keywords", interests.keywords)) from interests where basics_information.id = interests.resumeId),
                                "refreneces",(select json_arrayagg(json_object("name", references.name, "refrenece", references.reference)) from `references` where basics_information.id = `references`.resumeId),
                                "projects",(select json_arrayagg(json_object("name", projects.name, "description", projects.description, "startDate", projects.startDate, "endDate", projects.endDate,"url", projects.url, "entity", projects.entity, "type", projects.type, "highlights", projects.highlights, "keywords", projects.keywords, "roles", projects.roles)) from projects where basics_information.id = projects.resumeId)) as resume
